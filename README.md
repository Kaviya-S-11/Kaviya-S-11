# Hi, I'm **Kaviya** 👋
 
🎓 Computer Science Graduate (Fresher) | FrontEnd Developer | Angular Developer
I am a motivated FrontEnd developer with a strong foundation in HTML5, CSS3, JavaScript and hands-on experience in building responsive websites. Also Skilled in Python, MySQl

### 📌 Projects
- **Coffee Website**  
  Designed and developed a responsive coffee website using HTML, CSS, JavaScript.
  Demo Link: https://brewheaven-coffee-website.netlify.app/
  
- **Rock Paper Scissor**  
  Build a simple game using Python.

- **Calculator Application**  
  Desktop application built with Python and Tkinter.

### 🎯 Currently Looking For
- Front-end Developer (Fresher)
- Python Developer (Fresher)

 Reach me at
📫 Email: kaviyasakthivel115@gmail.com  
🔗 LinkedIn: linkedin.com/in/kaviya-sakthivel115

✨ *Learning & building step by step.*
